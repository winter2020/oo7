...
;; Motivation:
;; Same as with read.
(defmethod call-return (name buf len ret)
  (when (= name 'fgets)
    (untrust/region buf len)
    (untrust/region ret len)))
...
(defmethod call-return (name p n m ret)
  (when (is-in name 'fread '__fread_chk)
...
;; Rule:
;; recvfrom(_,p,n,...)
(defmethod call-return (name fd buf len flg src slen ret)
  (when (= name 'recvfrom)
    (untrust/region buf len)
    (untrust/value ret)))
...
;; User input
(defmethod call-return (name _ ret)
  (when (is-in name 'fgetc 'getc '_IO_getc)
    (untrust/value ret)))

;; fake taint source
(defmethod call-return (name ret)
  (when (= name 'get_taint_source)
    (untrust/value ret)))
...
(defmethod call (name s f v d e) 
    (when (is-in name '__isoc99_fscanf 'fscanf 'tfscanf) 
    (untrust/region v 4)))
...
(defmethod call-return (name b len ret)
  (when (= name 'getcwd)
    (untrust/region b len)
    (untrust/region ret len)))
...
(defmethod call-return (name ret)
  (when (is-in name 'getuid 'getegid 'getpid 'getegid 'geteuid)
    (untrust/value ret)))


(defmethod call(name f a e)
  (when (= name 'getsockname)
    (untrust/region a 16)))

(defmethod call(name f a e)
  (when (= name 'getpeername)
    (untrust/region a 16)))

;;(defmethod call-return (name ret)
;;  (when (is-in name '__ctype_b_loc)
;;    (untrust/region ret 0)))
