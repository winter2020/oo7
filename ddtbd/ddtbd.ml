...
let step3 pos ts ({state = S2 {cond; taint; load}} as h) =
  if Set.mem ts taint && Addr.(pos <> load)
  then Some {
      h with
      state = S3 {
          cond;
      ...
      ~f:(fun ({pdoms} as s) (name,entry,cfg) ->
          ...
          {s with pdoms =
                    ...
                    Seq.fold ~init:pdoms ~f:(fun pdoms b ->
                        Map.add pdoms ~key:(Block.addr b) ~data:pdom);
...
module Detector(Machine : Primus.Machine.S) = struct
  ...
  *)
  ...
  let promote_to_step3 pos ts =
    ...
    let step3 = promote (step3 pos ts) s.step2 in
    let locs =
  ...
  let access ~is_loaded (addr,value) =
    ...
    if Set.is_empty ts then Machine.return ()
    else if is_loaded
    then
...
module InputDetector(Machine : Primus.Machine.S) = struct
  ...
  let reflect_observation =
    Lisp.signal undefined_input @@ fun v -> Machine.return [v]
